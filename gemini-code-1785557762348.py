...
def init_db():
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS movies (code TEXT PRIMARY KEY, file_id TEXT, type TEXT, caption TEXT, views INTEGER DEFAULT 0, added_date TEXT)''')
...
# --- KINO QIDIRISH (Diqqat: StateFilter(None) qo'shildi) ---
...
async def search_movie(message: types.Message):
    ...
    conn = sqlite3.connect("kino_bot.db")
    cursor = conn.cursor()
    cursor.execute("SELECT file_id, type, caption FROM movies WHERE code=?", (code,))
    ...
    file_id, msg_type, caption = movie
...
async def top_movies(message: types.Message):
    conn = sqlite3.connect("kino_bot.db")
    cursor = conn.cursor()
    cursor.execute("SELECT code, caption, views FROM movies ORDER BY views DESC LIMIT 10")
    ...
    for i, (code, caption, views) in enumerate(movies, 1):
        cap_short = caption[:20] + "..." if caption and len(caption) > 20 else (caption or "Nomsiz")
        text += f"{i}. 🎬 <code>{code}</code> - {cap_short} (👁 {views} marta)\n"
...
async def latest_movies(message: types.Message):
    conn = sqlite3.connect("kino_bot.db")
    cursor = conn.cursor()
    cursor.execute("SELECT code, caption FROM movies ORDER BY added_date DESC LIMIT 5")
    movies = cursor.fetchall()
    conn.close()
    ...
    for code, caption in movies:
        ...
        text += f"🎬 Kod: <code>{code}</code> - {cap_short}\n"
...
async def random_movie(message: types.Message):
    conn = sqlite3.connect("kino_bot.db")
    cursor = conn.cursor()
    cursor.execute("SELECT code FROM movies ORDER BY RANDOM() LIMIT 1")
    movie = cursor.fetchone()
    conn.close()
...
# --- KINO YUKLASH (BU YERDA STATE ISHLATILADI) ---
...
async def add_movie_step3(message: types.Message, state: FSMContext):
    ...
    await message.answer("3️⃣ <b>Kino tavsifini yozing</b> (Yoki \"O'tkazib yuborish\" ni bosing):", parse_mode="HTML", reply_markup=markup)
...
async def add_movie_step4(message: types.Message, state: FSMContext):
    ...
    conn = sqlite3.connect("kino_bot.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO movies (code, file_id, type, caption, added_date) VALUES (?, ?, ?, ?, ?)", 
                   (data['code'], data['file_id'], data['type'], caption, date_now))
    conn.commit()
    conn.close()

    await message.answer(f"🎉 Kino bazaga muvaffaqiyatli saqlandi!\nKodi: <code>{data['code']}</code>", parse_mode="HTML", reply_markup=types.ReplyKeyboardRemove())
    await state.clear()
...
async def broadcast_start(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("📝 Tarqatmoqchi bo'lgan xabarni yuboring (Rasm, matn, video):")
