...
# ================= SOZLAMALAR =================
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8998624190:AAHWIIJW4pr3Hk2jRw_d0Ll6fVF4lwr7_4s")
MAIN_ADMIN_ID = int(os.environ.get("MAIN_ADMIN_ID", "8355669630"))
ADMIN_USERNAME = "smart_gemini"
DB_PATH = os.environ.get("DB_PATH", "/data/kino_bot.db")   # Railway Volume ulangan bo'lsa shu yerda saqlanadi
_db_dir = os.path.dirname(DB_PATH)
if _db_dir and not os.path.isdir(_db_dir):
    try:
        os.makedirs(_db_dir, exist_ok=True)
    except Exception:
        DB_PATH = "kino_bot.db"  # volume mavjud bo'lmasa, joriy papkaga tushamiz
...
def init_db():
    ...
    # avval noto'g'ri (masalan @username) formatda saqlangan kanal linklarini tuzatib qo'yamiz
    for ch in db_all("SELECT chat_id, url FROM channels"):
        fixed = normalize_channel_url(ch["url"])
        if fixed and fixed != ch["url"]:
            db_exec("UPDATE channels SET url=? WHERE chat_id=?", (fixed, ch["chat_id"]))
...
def channels_menu_markup():
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔍 Foydalanuvchi obunasini tekshirish", callback_data="adm_check_user_sub")],
...
async def get_sub_markup(user_id):
    ...
    buttons = []
    for ch in channels:
        url = normalize_channel_url(ch["url"]) or ""
        # Telegram faqat http/https URL qabul qiladi; noto'g'ri bo'lsa tugma qo'shilmaydi
        # (lekin obuna tekshiruvi baribir chat_id orqali davom etadi)
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if not ok:
        ...
        try:
            ...
            # Kanal linklaridan biri noto'g'ri bo'lsa ham bot yiqilmasin,
            # tugmasiz oddiy matn bilan davom etadi.
            await message.answer(
                f"👋 Salom, {message.from_user.full_name}!\n\n"
                f"{get_setting('sub_text')}\n\n"
...
def normalize_channel_url(raw: str):
    """Kanal linkini har doim to'g'ri https://t.me/... formatiga o'tkazadi.
    @username, t.me/username, oddiy username yoki tayyor havolani qabul qiladi.
    Noto'g'ri (bo'sh yoki formatlab bo'lmaydigan) bo'lsa None qaytaradi."""
    if not raw:
        return None
    raw = raw.strip()
    ...
    # oddiy username (masalan "mychannel")
...
async def adm_add_channel_url(message: types.Message, state: FSMContext):
    url = normalize_channel_url(message.text.strip())
    if not url:
        await message.answer(
            "❌ Noto'g'ri link formati. Iltimos qaytadan yuboring.\n"
            "Masalan: https://t.me/kanalim yoki https://t.me/+AbCdEfGh (yopiq kanal)"
        )
        return
    await state.update_data(url=url)
    await message.answer(f"✅ Link qabul qilindi: {url}\n📝 Endi kanal nomini kiriting:")
...
async def adm_list_channels(callback: types.CallbackQuery):
    ...
    if not channels:
        ...
        return
    me = await bot.get_me()
    text = "📋 <b>Kanallar ro'yxati va bot holati:</b>\n\n"
    for c in channels:
        tur = "🔒 Yopiq" if c["is_private"] else "🔓 Ochiq"
        text += f"{tur} | {c['name']} | <code>{c['chat_id']}</code>\n{c['url']}\n"
        try:
            bot_member = await bot.get_chat_member(chat_id=int(c["chat_id"]), user_id=me.id)
            if bot_member.status in ("administrator", "creator"):
                text += "✅ Bot shu kanalda ADMIN — tekshiruv to'g'ri ishlaydi\n\n"
            else:
                text += f"⚠️ Bot bu kanalda ADMIN EMAS (holat: {bot_member.status}) — obuna/so'rov tekshiruvi ISHLAMASLIGI MUMKIN!\nBotni shu kanalga ADMIN qilib qo'ying.\n\n"
        except Exception as e:
            text += f"❌ Bot bu kanalni umuman ko'ra olmayapti ({e}).\nBotni shu kanalga a'zo/admin qilib qo'shing va chat_id to'g'riligini tekshiring!\n\n"
    await callback.message.answer(text, parse_mode="HTML")


@dp.callback_query(F.data == "adm_check_user_sub")
async def adm_check_user_sub(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.answer("🔍 Tekshiriladigan foydalanuvchi ID sini kiriting:")
    await state.set_state(AdminState.waiting_check_sub_id)


@dp.message(AdminState.waiting_check_sub_id)
async def adm_check_user_sub_step2(message: types.Message, state: FSMContext):
    try:
    ...
    if not channels:
        ...
        await state.clear()
        return
    text = f"🔍 <b>Foydalanuvchi {uid} uchun obuna holati:</b>\n\n"
    overall_ok = True
    for c in channels:
        line = f"📢 {c['name']} (<code>{c['chat_id']}</code>): "
        member_ok = False
        try:
            member = await bot.get_chat_member(chat_id=int(c["chat_id"]), user_id=uid)
            member_ok = member.status in ("member", "administrator", "creator", "restricted")
            line += f"a'zolik holati = {member.status}"
        except Exception as e:
            line += f"❌ tekshirib bo'lmadi ({e})"
        req = db_one("SELECT 1 FROM join_requests WHERE user_id=? AND chat_id=?", (uid, c["chat_id"]))
        line += f" | so'rov yuborilganmi: {'✅ ha' if req else '❌ yoq'}"
        satisfied = member_ok or bool(req)
        line += f" → {'✅ OK' if satisfied else '❌ TALAB QILINADI'}"
        if not satisfied:
            overall_ok = False
        text += line + "\n\n"
    text += f"\n<b>Umumiy natija:</b> {'✅ Barcha shartlar bajarilgan' if overall_ok else '❌ Kamida bitta kanal shartini bajarmagan'}"
    await message.answer(text, parse_mode="HTML")
    await state.clear()
