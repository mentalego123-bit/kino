...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO channels (channel_id, url, name, is_private) VALUES (?, ?, ?, ?)",
              ("-100_MAXFIY_ID_SHU_YERGA_YOZING", "https://t.me/+t30YnzAM5iFlNGVk", "Yopiq Kanal", 1))
    
    conn.commit()
    conn.close()

# ================= FSM HOLATLAR =================
class AdminState(StatesGroup):[cite: 5]
...
class UserState(StatesGroup):[cite: 5]
...
async def join_request_handler(chat_join: types.ChatJoinRequest):[cite: 5]
...
async def start_cmd(message: types.Message):[cite: 5]
...
async def order_movie_step1(message: types.Message, state: FSMContext):[cite: 5]
...
async def vip_management_menu(callback: types.CallbackQuery):[cite: 5]
    conn = get_db()
    c = conn.cursor()
...
async def users_file_generate(callback: types.CallbackQuery):[cite: 5]
    conn = get_db()
