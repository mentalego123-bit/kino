...
def init_db():
    ...
    # Majburiy kanallarni standart qo'shish (eski bazaga ta'sir qilmaydi, faqat yo'q bo'lsa qo'shadi)
...
# ================= ADMIN PANELI QAYTA CHAQIRISH UCHUN FUNKSIYA =================
...
async def edit_ad_text_step2(message: types.Message, state: FSMContext):
    ...
    conn.close()
    await state.clear()
    await send_admin_panel(message)  # Asabni asrash uchun tugma avtomat chiqadi
...
async def add_p_channel_step3(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        name, url = text.split("|", 1)  # Xatolik uchun FIX qo'llanildi
...
async def manage_admins(callback: types.CallbackQuery, state: FSMContext):
    if callback.fromuser.id != MAIN_ADMIN_ID:
...
async def main():
    ...
    # Veb serverni fonda ishga tushirish (Render va hokazolar uchun portni band qiladi)
