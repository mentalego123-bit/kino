...
def normalize_url(raw: str):
    ...
    Noto'g'ri (formatlab bo'lmaydigan) bo'lsa None qaytaradi."""
    if not raw:
        return None
    raw = raw.strip()
...
def init_db():
    ...
    cursor.execute("INSERT OR IGNORE INTO private_channels (channel_id, url, name) VALUES ('-1003297745646', 'https://t.me/+t30YnzAM5iFlNGVk', 'Yopiq Kanal')")
    ...
    # avval noto'g'ri (masalan @username yoki bo'sh) formatda saqlangan linklarni tuzatib qo'yamiz
    for ch_id, url in cursor.execute("SELECT channel_id, url FROM channels").fetchall():
        fixed = normalize_url(url)
        if fixed and fixed != url:
            cursor.execute("UPDATE channels SET url=? WHERE channel_id=?", (fixed, ch_id))
    for ch_id, url in cursor.execute("SELECT channel_id, url FROM private_channels").fetchall():
        fixed = normalize_url(url)
        if fixed and fixed != url:
            cursor.execute("UPDATE private_channels SET url=? WHERE channel_id=?", (fixed, ch_id))
    conn.commit()
    conn.close()
...
async def get_sub_markup(user_id):
    ...
    for name, url in unsubscribed:
        ...
        # (aks holda bot "inline keyboard button URL invalid" xatosi bilan yiqiladi)
        if fixed_url and (fixed_url.startswith("http://") or fixed_url.startswith("https://")):
            markup.inline_keyboard.append([InlineKeyboardButton(text=f"📢 {name}", url=fixed_url)])
...
async def search_movie(message: types.Message):
    ...
    if markup:
        try:
            ...
            await message.answer("Avval kanallarga obuna bo'ling! (/start ni bosing)")
        return
...
async def add_p_channel_step3(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if not url:
            await message.answer("❌ Havola formati noto'g'ri. Qaytadan yuboring.\nFormat: <code>Nomi|https://t.me/+joinlink</code>", parse_mode="HTML")
            return
        data = await state.get_data()
        
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO private_channels (channel_id, url, name) VALUES (?, ?, ?)", (data['ch_id'], url, name.strip()))
        conn.commit()
        conn.close()
...
async def add_channel_step3(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO channels (channel_id, url, name) VALUES (?, ?, ?)", (data['ch_id'], url, name.strip()))
        conn.commit()
        conn.close()
